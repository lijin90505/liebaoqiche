	...
	<script type="text/javascript">
		require(['jquery','common'], function($){
			...
			$("#btnCreate").on('click', function(){
				window.location.href = '${ctx}/accesssystem/couponPermissionAssignment/add';
			});
			...
			$("#btnDistribution").on('click', function(){
				...
				if (selected.length == 1) {
					showDialog('分配', '${ctx}/accesssystem/couponPermissionAssignment/permission?id=' + selected[0].id, 800, 550, function () {
			...
			$("#btnUpdate").on('click', function(){
				...
				if(selected.length == 1){
					window.location.href = '${ctx}/accesssystem/couponPermissionAssignment/edit?id=' + selected[0].id;
				}
			});
				...
				if(selected.length == 0){
                    ...
                    $.messager.confirm('警告', '确认删除本条记录吗?', function (r) {
                        if(r){
                            ...
                            $.ajax({
	                            url: '${ctx}/accesssystem/couponPermissionAssignment/delete',
	                            data: {
	                                ids: ids
	                            },
	                            type: 'post',
