...
<div style="height: 500px;margin:10px 20px;box-sizing:border-box;">
	<div class="easyui-layout" data-options="fit:true">
		...
		<div data-options="region: 'west', collapsible:false" title="未选积分规则"
		     style="width: 350px;">
			<table id="Grid_UnchoosedPayChannels" class="easyui-datagrid"
				...
				<tr>
					...
					<th data-options="field:'pointRuleName',width:120">积分规则名称</th>
		...
		<div data-options="region: 'east', collapsible:false" title="已有积分规则"
		     style="width: 350px">
...
<script type="text/javascript">
	require(['jquery','common'], function($){
		...
		$(function(){
			// 加载角色授权信息
			$.ajax({
				url: '${ctx}/accesssystem/accessSystem/sysPointRule',
				data: {
				...
				success: function (json) {
					...
					if(json && json.sysUnChoosedPayChannelList){
						$("#Grid_UnchoosedPayChannels").datagrid('loadData', json.sysUnChoosePointRuleList);
					}

					// 显示已经选中的角色
					if(json && json.sysChoosedPayChannelList){
						$("#Grid_choosedPayChannels").datagrid('loadData', json.sysChoosePointRuleList);
					}
		...
		$('#btnSave_SysUserAccredit').click(function () {
			$.messager.confirm("授权确认","确认要给该系统进行授权？",function(r){
				if(r){
					...
					$.ajax({
						url: '${ctx}/accesssystem/accessSystem/accreditPoint',
						data: {
							systemSign : systemSign,
							allPointRulesId : allChannelId
						},
						type: 'post',
						dataType: 'json',
						success: function (json) {
		...
		$("#btnRight_SysSelectChannel").click(function(){
			...
			for(var i=0;i<selectedChannels.length;i++){
				...
				$("#Grid_choosedPayChannels").datagrid('insertRow', {
					row:{
						pointRuleName : selectedChannels[i].pointRuleName ,
		...
		$("#btnDoubleRight_SysUserSelectRole").click(function(){
			...
			for(var i=0;i<allUnchoosedChannels.rows.length;i++){
				...
				$("#Grid_choosedPayChannels").datagrid('insertRow', {
					row:{
						pointRuleName : allUnchoosedChannels.rows[i].pointRuleName ,
		...
		$("#btnDoubleLeft_SysUserSelectRole").click(function(){
			...
			for(var i=0;i<allChoosedChannels.rows.length;i++){
				...
				$("#Grid_UnchoosedPayChannels").datagrid('insertRow', {
					row:{
						pointRuleName : allChoosedChannels.rows[i].pointRuleName ,
