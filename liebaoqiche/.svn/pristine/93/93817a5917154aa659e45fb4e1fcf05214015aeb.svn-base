			...
			<table id="Grid" class="easyui-datagrid" title="" ctrlSelect="true" striped="true"
			    ...
			    <thead>
			        <tr>
			            ...
			            <th data-options="field:'access',width:100,formatter:function(value,row,index){
							if (row.access ==''){
								return '后台操作';
												} else{
											return row.access;
			            ...
			            <th data-options="field:'type',width:100,formatter:function(value,row,index){
			            	if(row.type =='00'){
			            		return '获取';
			            	}else if(row.type =='01'){
			            		return '使用';
			...
			<div id="Grid_Toolbar">
				<form id="searchForm" name="searchForm">			        
					...
					<div class="pull-right">
						...
						<shiro:hasPermission name="userExperience:operation">
					        <a id="btnOperation" href="javascript:void(0)" class="easyui-linkbutton button-default"><i class="fa fa-credit-card fa-lg"></i>&nbsp;&nbsp;经验值增减</a>
						</shiro:hasPermission>
				    </div>
				</form>
		    </div>
	...
	<script type="text/javascript">
		require(['jquery','common'], function($){
			...
			$('#btnOperation').on('click',function(){
				...
				showDialog('操作', '${ctx}/experience/userExperience/experienceQuery', 800, 500, function () {
					$('#Grid').datagrid('reload');
				});
			})
