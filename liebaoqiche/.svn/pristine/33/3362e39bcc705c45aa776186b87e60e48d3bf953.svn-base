	...
	<div class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'center'">
			<table id="Grid" class="easyui-datagrid" title="" ctrlSelect="true" striped="true"
				...
				data-options="url:'${ctx}/card/couponRecall/list',fit:true, fitColumns:true, checkOnSelect:true, selectOnCheck:true, border: false">
			    <thead>
			        <tr>
			            <th data-options="field:'ck',checkbox:true,hidden:true"></th>
			            <th data-options="field:'id',hidden:true">id</th>
			            <th data-options="field:'couponName'">卡券名称</th>
			            <th data-options="field:'signName'">系统名称</th>
			            <th data-options="field:'recallNum'">数量</th>
			            <th data-options="field:'recallUserId',hidden:true">recall_user_id</th>
			            <th data-options="field:'recallUsername'">用户名称</th>
			            <th data-options="field:'signId',hidden:true">sign_id</th>
			...
			<div id="Grid_Toolbar">
				<form id="searchForm">
					<shiro:hasPermission name="couponRecall:query">
						...
						<input class="easyui-combobox" id="status" name="status"
						       data-options="
									        ...
									        editable:false,
									        prompt:'卡券发放状态'">
						<input class="easyui-combobox" id="recallUserId" name="recallUserId"
							       data-options="
					...
					<div class="pull-right">
						<shiro:hasPermission name="couponRecall:create">
					        <a id="btnCreate" href="javascript:void(0)" class="easyui-linkbutton button-success"><i class="fa fa-plus-circle fa-lg"></i>&nbsp;&nbsp;撤回</a>
						</shiro:hasPermission>

						<shiro:hasPermission name="couponRecall:update">
					        <a id="btnUpdate" href="javascript:void(0)" class="easyui-linkbutton button-default"><i class="fa fa-pencil fa-lg"></i>&nbsp;&nbsp;修改</a>
						</shiro:hasPermission>

						<shiro:hasPermission name="couponRecall:delete">
	...
	<script type="text/javascript">
		require(['jquery','common'], function($){
			...
			$("#btnCreate").on('click', function(){
				showDialog('撤回', '${ctx}/card/couponRecall/add', 800, 500, function () {
			...
			$("#btnUpdate").on('click', function(){
				...
				if(selected.length == 1){
					showDialog('编辑', '${ctx}/card/couponRecall/edit?id=' + selected[0].id, 800, 500, function () {
						$('#Grid').datagrid('reload');
					});
				}
			});
				...
				if(selected.length == 0){
                    ...
                    $.messager.confirm('警告', '确认删除本条记录吗?', function (r) {
                        if(r){
                            ...
                            $.ajax({
	                            url: '${ctx}/card/couponRecall/delete',
	                            data: {
	                                ids: ids
	                            },
	                            type: 'post',
