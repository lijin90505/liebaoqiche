...
<div style="height: 500px;margin:10px 20px;box-sizing:border-box;">
	<div class="easyui-layout" data-options="fit:true">
		...
		<div data-options="region: 'west', collapsible:false" title="未选支付渠道"
		     style="width: 350px;">
			<table id="Grid_UnchoosedPayChannels" class="easyui-datagrid"
				...
				<tr>
					...
					<th data-options="field:'payChannelTypeDesc',width:120">支付渠道名称</th>
		...
		<div class="btnLink" data-options="region:'center', border: false">
			<a id="btnRight_SysSelectChannel" href="javascript:void(0)"
			   class="easyui-linkbutton button-default"><i
				...
				id="btnDoubleRight_SysUserSelectChannel" href="javascript:void(0)"
				class="easyui-linkbutton button-default"><i
				class="fa fa-angle-double-right fa-lg"></i></a> <a
				id="btnDoubleLeft_SysUserSelectChannel" href="javascript:void(0)"
				class="easyui-linkbutton button-default"><i
				class="fa fa-angle-double-left fa-lg"></i></a> <a
				id="btnLeft_SysUserSelectChannel" href="javascript:void(0)"
				class="easyui-linkbutton button-default"><i
				class="fa fa-angle-left fa-lg"></i></a>
		</div>
		<div data-options="region: 'east', collapsible:false" title="已有支付渠道"
		     style="width: 350px">
...
<script type="text/javascript">
	require(['jquery','common'], function($){
		...
		$(function(){
			...
			$.ajax({
				...
				success: function (json) {
					...
					// 显示未选中的支付渠道
					if(json && json.sysUnChoosedPayChannelList){
						$("#Grid_UnchoosedPayChannels").datagrid('loadData', json.sysUnChoosedPayChannelList);
					}

					// 显示已经选中的支付渠道
					if(json && json.sysChoosedPayChannelList){
						$("#Grid_choosedPayChannels").datagrid('loadData', json.sysChoosedPayChannelList);
					}
		...
		$("#btnRight_SysSelectChannel").click(function(){
			...
			for(var i=0;i<selectedChannels.length;i++){
				...
				$("#Grid_choosedPayChannels").datagrid('insertRow', {
					row:{
						payChannelTypeDesc : selectedChannels[i].payChannelTypeDesc ,
		...
		//点击插入选择所有支付渠道
		$("#btnDoubleRight_SysUserSelectChannel").click(function(){
			...
			for(var i=0;i<allUnchoosedChannels.rows.length;i++){
				...
				$("#Grid_choosedPayChannels").datagrid('insertRow', {
					row:{
						payChannelTypeDesc : allUnchoosedChannels.rows[i].payChannelTypeDesc ,
		...
		//点击移除选择的N行
		$("#btnLeft_SysUserSelectChannel").click(function(){
		...
		//点击移除选择的所有角色
		$("#btnDoubleLeft_SysUserSelectChannel").click(function(){
			...
			for(var i=0;i<allChoosedChannels.rows.length;i++){
				...
				$("#Grid_UnchoosedPayChannels").datagrid('insertRow', {
					row:{
						payChannelTypeDesc : allChoosedChannels.rows[i].payChannelTypeDesc ,
